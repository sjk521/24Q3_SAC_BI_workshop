...
class ChartPlotareaComponent extends HTMLElement {
    ...
    constructor() {
        ...
        this._rounded = true;
        this._donutDataLabel = false;
        this._dataMarkerSize = 100;
        this._donutDataLabelSize = 100;
        this._axisLabelColor = '#333';
        this._tenkDataLabel = true;
        this._inclined = true;
        this._customUnit = false;
        this._customUnitNum = 1;
        this._customUnitName = '';
        this._imgFill = false;
        this._img = '';
        this.flag = false;
        this._axisLabelIcon = false;
        this._format = 'typeA';
    ...
    renderData(dataInfo, options) {
        ...
        // img fill
        let backgroundImage = '';
        if(this._imgFill && this._img)
            backgroundImage = `background-image: url(${this._img}); background-size: auto ${height}px;`;

        barColumnContainer.setAttribute(
            'style',
            `${backgroundImage} ${barStyle} position: absolute; top: ${y}px; left: ${x}px; width: ${width}px; height: ${height}px;${dataInfo.opacity !== undefined ? `opacity: ${dataInfo.opacity};` : ''}`
        );
    ...
    renderLabel(labelInfo, options)  {
        ...
        if (this._donutDataLabel && this._chartType === 'barcolumn' && options.markerSize > 0 && !isNaN(options.totalSeriesValue)) {
            ...
            var textInMillian;
            if(this._customUnit){
                let resultValue = (labelInfo.pointValue / this._customUnitNum).toFixed(0);
                textInMillian = `${resultValue} ${this._customUnitName}`;
            } else
                textInMillian = getTextInMillian(pointValue);
        ...
        } else if(this._chartType == 'stackedbar' && this._absolute) {
            labelElement = document.createElement('span');
            labelElement.classList.add('common-label');
            labelElement.setAttribute(
                'style',
                `background-color: ${bgColor}; position: absolute; top: ${y}px; left: ${x}px; width: ${width}px; height: ${height}px; color: ${color}; font-size: ${fontSize};`
            );
            labelElement.innerHTML = labelInfo.pointValue;
        } else if(this._customUnit) { // 일반적인 케이스 custom unit이 켜져 있으면 숫자 유형 적용
            labelElement = document.createElement('span');
            labelElement.classList.add('common-label');
            labelElement.setAttribute(
                'style',
                `background-color: ${bgColor}; position: absolute; top: ${y}px; left: ${x}px; width: ${this._size.width - x}px; height: ${height}px; color: ${color}; font-size: ${fontSize}; text-align: center`
            );
            let resultValue = (labelInfo.pointValue / this._customUnitNum).toFixed(2);
            ...
            switch(this._format){
                case 'typeA':
                    ...
                    break;
                case 'typeB':
                    resultValue = resultValue.toLocaleString('de-DE');
                    break;
                case 'typeC':
                    resultValue = resultValue.toLocaleString('fr-CA');
                    break;
            }
            labelElement.innerHTML = `${resultValue} ${this._customUnitName}`;
        } else {
            // Don't display the label if user don't want to use donut data label and
            // current label is hidden or out of viewing range
            if (!(this._inclined && this._chartType === "line") && (hidden || outOfViewport)) {
                return;
            }
        ...
        if(this._inclined && this._chartType === "line"){
            // labelElement.style.transform = "translateY(-6px) rotate(-40deg)";
            if(this.flag)
    ...
    /**
     * User settings to set if data label should be inclined for line chart
     */
    set inclined(value) {
        this._inclined = value;
        this.render();
    }

    /**
     * User settings to set if data label should use custom unit
     */
    ...
    /**
     * User settings to set if data marker should be filled with img
     */
    set imgFill(value) {
        this._imgFill = value;
        this.render();
    ...
    /**
     * User settings to set if axis label icon should be displayed
     */
    set axisLabelIcon(value) {
        this._axisLabelIcon = value;
        this.render();
    }

    /**
     * User settings to set if data label is showed in absolute value
     */
    set absolute(value) {
    ...
    /**
     * User settings to set number formatting for custom unit
     */
    set format(value) {
        this._format = value;
        this.render();
